# Genomic Association Risk Score for Admixed Populations - GARSA

GARSA was develped using Pytohon3 and aims to semi-automate the main steps for Genome Wide Association Studies and Polygenic Risk Score analysis
...
reamember to activate the environment before running the GWAS module with the bolt-lmm flag
```
...
---------------------------------------------------------------------------------------------------------------------------------------------
 ...
 #### With that no further adjusts in the dataset will be necesseray further in the analysis
...
`python3 main_script_FOXCONN.py` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
`python3 main_script_FOXCONN.py desdup`--> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
Usage:
`python3 main_script_FOXCONN.py desdup -vcf chr22_pop1.vcf.gz` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA

#### Update rsIDs module (SNP annotation)

`python3 main_script_FOXCONN.py update_rsID` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
Exemplo de uso:
`python3 main_script_FOXCONN.py update_rsID -vcf chr22_pop1.vcf.gz`
...
`python3 main_script_FOXCONN.py rename_sample_id` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
`python3 main_script_FOXCONN.py quality_control` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
The flag `-vcf` is required, and also the flags ` -geno -maf -R2 --score-type and -use_HWE` are important for the user to pay attaion  

Usage:
`python3 main_script_FOXCONN.py quality_control -vcf chr22_pop1.vcf.gz -bcftools path/to/bcftools/executable -plink2 path/to/plink2/executable -plink path/to/plink/executable`
...
This script works as a loop, and the `{1..22}` will iterate over the fileas as such:
...
This module executes quality control on samples, including heterozigozity rates

`python3 main_script_FOXCONN.py quality_ind` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
Usage:
`python3 main_script_FOXCONN.py quality_ind -vcf output_file_merged.vcf.gz` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA

## Kinship with corretion to admixture populations

This is an important module, aiming the kisnhip analysis with corrections for admixture populations. As proposed by [Conomos et. al (2016)](https://www.cell.com/ajhg/fulltext/S0002-9297(15)00496-6)

`python3 main_script_FOXCONN.py kinship` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
Here 3 main outputs are generated and can be analysed by the user:

1. Kinship_corrected.tsv --> Kinship table (all against all) with corrections for admixture populations
...
4. Run a "DeNovo" admixture analysis for identification of best N of populations --> this generates a colored graphical output that the user can check for the number of informative PCs

`python3 main_script_FOXCONN.py PCA` --> LEMBRAR DE MUDAR O NOME DO ARQUIVO PARA O NOME FINAL DA FERRAMENTA
...
2. file_name>_PCA_total.txt --> Outptu with all the information about the PCs for related and unralted samples
3. PC_plots_PCA1.pdf --> File with all PCA plots for user visualization 


## GWAS module

`python3 main_script_FOXCONN.py GWAS`
...
We recommend the use of GCTA for populations with number of samples < 5000 and BOLT-LMM for populations with number of samples > 5000  

Importante to notice that BOLT-LMM calculates it's own kinship matrix and do not provide a way to input the corrected one calculated above.
...
In this example only "sex" is used as qualitative covarble. Important: For this analysis, if the user whish to use more covariables, order them after the "sex" covariable on the file (keeping it on the third columns as showed below).
...
qcovar file (quantitavie covriable):

On this file we use the PCs generated above, and add to thath file age, age squared (used by many authors), BMI and other quantitative traces that might be important
...
`python3 main_script_FOXCONN.py PRS`
...
Ex. usage: script.py
```
On this step the user must provide all the files (covar, qcovar and phenotype) used on the GWAS step -- Important: use the flag `-n_pcs` to provide the number of PCs used on the step above.
