...
#define fread_check(ptr, size, nmemb, stream) (assert(fread((ptr), (size), (nmemb), (stream)) == (nmemb)))
#define fseek_check(stream, offset, whence) (assert(fseek((stream), (offset), (whence)) == 0))
...
namespace FileUtils {
  ...
  std::vector < std::pair <std::string, std::string> > readSampleIDs(const std::string &file);
